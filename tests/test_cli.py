...
def _build_cmd():
    """Build command to run the CLI. Prefer using `uv run` if available, otherwise use the current
    Python executable. Pass --help to avoid interactive prompts and make the test minimal.
    """
...
def test_cli_runs_help():
    ...
    # Expected help substrings. Each entry is a list of acceptable variants for the same feature.
    # For example the list command may be named --task_list or --tasks_list depending on implementation.
    expected_groups = [
        ["--create_db"],
        ["-h, --help"],
        ["--task_add"],
    ...
    assert not missing_groups, (
        "The CLI help output is missing expected option(s): " + ", ".join(missing_groups)
        + f"\n\nFull help output:\n{output}"
    )
