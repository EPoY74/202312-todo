...
def work_with_slq(DB_NAME: str, db_sql_query: str, db_sql_data: str = None ):  # Далаем запись в БД
    """
    Пишет запрос в базу данных. Если указаана только БД и запрос - то выполняем только его
    Если укзазан БД, запрос и данные - то выполняем и данные и запрос.
    """
    logging.debug("Функция записи в БД")
    
    DB_NAME_RW = "file:" + DB_NAME + "?mode = rw"  # Открываем БД в режиме записи
