...
def make_task(text_of_task: str):  # Создаю таск в БД
    ...
    try:
        with sql3.connect(db_name_rw, uri=True) as db_connection:
            db_cursor = db_connection.cursor()
            # db_sql_query = f'INSERT INTO my_todo_list
            # (data_of_creation, todo_text, is_gone) VALUES (?, {text_of_task}, ?)'
...
def task_gone(task_gone_id: int) -> None:  # isGone Помечаем таск исполненым
    """
    Автор: Евгений Петров, Челябинск, p174@mail.ru
    ...
    list_of_tasks("one", task_gone_id)  # Показываеи запись до их изменения
    ...
    if confirm_action("пометить исполненным задание ", id_and_date):
        ...
        data.work_with_data("write", "one", select_id_sql_gone)  # Помечаем запись выполненой
