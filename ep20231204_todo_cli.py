...
def set_tasks_deadline(task_deadline_id: int):  # -done- Устанавливаем дату исполнения
    ...
    # TODO Сделать проверку установлена ли крайняя дата выполнения и
    #  если установлена уточнить, меняем или нет
    # TODO Сделать проверку, больше ли вводимая дата текущего числа
    # TODO Написать на почту, если кто-то попытается поменять дату
    #  исполнения на прошедшую (а надо ли?)
    # TODO Проверить на завершенность - если завершено,
    ...
    while True:
        ...
        try:
            ...
            logger.error("set_tasks_deadline():"
                         " Пользователь ввел некорректное значение даты и времени")
            continue
...
def confirm_action(confirm_text: str = "---Текст---",
                   other_text: str = None):  # isGone Проверяет выполнение операции
    """
    Автор: Евгений Петров, Челябинск, p174@mail.ru
    ...
    other_text - возможность добавить какой-то текст,
     кроме описания операции(например номер позиции)
    """
...
def main_body():  # isGone Основная логика программы (выбор действия с заданиями)
    ...
    parser.description = ("Программа создает ToDo"
                          " список дел в текстовом консольном режиме.")
    parser.add_argument("--create_db",
                        help="Создаем базу данных для списка задач", action="store_true")
    parser.add_argument("--task_add",
                        type=str,
                        help="""Описание задачи, которую заводим: --task_add \"Это запись\" """)
    parser.add_argument("--task_deadline",
                        type=int,
                        help="Устанавливает дату, до которой надо выполнить задание:"
    ...
    parser.add_argument("--task_delete",
                        type=int,
                        help="Удаляет запись с номером:"
...
def print_help_info():
    ...
    print("--task_add:"
          ...
          " --task_add \"Это запись\" ")
    print("--task_deadline:"
          " Устанавливает дату, до которой надо выполнить задание:"
          " --task_deadline номер_записи")
    print("--task_list: Выводит список задач")
    print("--task_done:"
          " Помечает задание с номером № завершенным:"
